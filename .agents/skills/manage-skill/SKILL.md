...
- Each skill or canonical reference uses its family's structure and has one owner, explicit effects, and useful instructions only.
...
- Catalogs, canonical references, generated bundles, templates, and checks agree; the report distinguishes execution, source simulation, and remaining gaps.
...
3. For authorized changes, use the [family templates](references/templates.md). Give each instruction a concrete effect; replace vague prohibitions with a specific failure and alternative. Preserve mode boundaries and evidence requirements while removing duplication. Edit shared procedures only in canonical references; declare conditional workflow links, then regenerate their bundles.
4. Self-assess, then use [Quality Validation](../../../references/protocols/quality-validation.md) with the [review](references/review.md), [critique](references/critique.md), and [coordination](references/coordination.md) perspectives. If the reference is unavailable, run those perspectives and the repair loop directly. Test individual skills and isolated installations, conditional loading, and handoffs, including unsupported claims and unintended effects.
