...
- `capabilities/`: reusable internal procedures, named `capability-<domain>-<function>`.
- Each skill owns its `SKILL.md` and relevant bundled references. Load references on demand.
...
Use `build`, `deliver`, `explain`, `explore`, `improve`, `manage`, `plan`, or `review` for workflows. Reuse these verbs before introducing another. Capability domain and function are nouns; never abbreviate `capability`. Mandatory entries use `vllnt-<subject>-principles`.
...
2. Run `bash scripts/test-validate-frontmatter.sh`, `bash scripts/validate-frontmatter.sh`, `bash scripts/test-pre-commit.sh`, `python3 scripts/test-validate-docs.py`, `python3 scripts/validate-docs.py`, and `git diff --check`.
...
Skills are Markdown; no package build or new runtime dependency is needed. Existing maintenance scripts remain independently testable.
