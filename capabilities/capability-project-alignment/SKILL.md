...
3. Use `capability-stack-packages` for current source, registry, and consumer evidence when available. Otherwise gather equivalent scoped evidence: selected manifests, paginated source or registry results, exact latest or canary metadata, and consumer declarations or lock resolutions.
