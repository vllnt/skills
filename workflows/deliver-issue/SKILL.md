...
2. Inspect contracts, callers, tests, and configuration. In planning mode, return the implementation plan, gates, and proposed checks only. In delivery mode, implement the smallest authorized change and exercise relevant success and failure behavior.
3. For a substantive candidate, use `capability-quality-validation` when available; otherwise apply its feedback loop directly. Triage evidence-backed findings, repair only within authority and mode, and renew affected checks and verdicts.
