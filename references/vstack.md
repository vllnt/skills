...
3. Use `build-project` for a new baseline and `improve-project` for adapting an existing project. Both conditionally use [Project alignment](capabilities/project-alignment/REFERENCE.md) for target decisions and [Stack packages](capabilities/stack-packages/REFERENCE.md) for live package evidence, with standalone fallbacks.
