...
| C49 | Pass accepted review and tests into landing unchanged; then vary the candidate, base, inputs, environment, or requirements. Repeat with a general validation acceptance that does not evidence security or operational review, and with a missing required local check. | Retain valid scoped acceptance without repeating evidenced perspectives; review unevidenced perspectives and uncovered scope, and refresh invalidated proof. General acceptance does not prove complete review coverage. Accepted review never substitutes for a missing required local result: the push stays blocked. |
...
| C81 | Code Tests verifies a denial contract, but the assertion ends before the operation completes. A controlled negative case introduces a forbidden effect after the assertion ends. | Reject the incomplete proof. Observe the relevant state and effects through the contract's completion boundary, and demonstrate that the negative case fails the check. A short wait does not prove the bounded contract or all future timing behavior. |
| C82 | A Code Tests verification handoff loses a required check's output during cleanup, while an independent current check's report remains readable. | Keep the valid independent evidence. Recover retained candidate-bound proof or rerun the affected check; otherwise leave its required proof pending. Retain acceptance-relevant outputs outside cleanup. |
